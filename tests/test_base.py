...
def test_tree_label(X, tree):
    ...
    tdata = td.TreeData(X, obst={"0": tree, "1": tree}, label="tree", allow_tree_overlap=True)
...
def test_tree_overlap(X, tree):
    ...
    with pytest.raises(ValueError):
        tdata = td.TreeData(X, obst={"0": tree, "1": second_tree}, allow_tree_overlap=False)
    # Test overlap allowed
    tdata = td.TreeData(X, obst={"0": tree, "1": second_tree}, allow_tree_overlap=True)
